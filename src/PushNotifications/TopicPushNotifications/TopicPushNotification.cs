...
namespace PushNotifications
{
    public class TopicPushNotification : AggregateRoot<TopicPushNotificationState>
    {
        ...
        public TopicPushNotification(TopicPushNotificationId id, Topic topic, NotificationPayload notificationPayload, Dictionary<string, object> notificationData, Timestamp expiresAt, bool contentAvailable)
        {
            if (StringTenantId.IsValid(id) == false) throw new ArgumentException(nameof(id));
            if (ReferenceEquals(null, topic) == true) throw new ArgumentException(nameof(topic));
            ...
            IEvent @event = new TopicPushNotificationSent(id, topic, notificationPayload, notificationData, expiresAt, contentAvailable);
            Apply(@event);
        }
    }
}
