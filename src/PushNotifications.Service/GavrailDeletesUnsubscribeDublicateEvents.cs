...
namespace PushNotifications.Service
{
        ...
        ISignalHandle<GavrailDeletesUnsubscribeDublicateEventsSignal>
    {
        ...
        private static byte[] deviceSubscriptionRootNameAsBytes = Encoding.UTF8.GetBytes(":subscription:");
                ...
                OnAggregateStreamLoadedAsync = async arStream =>
                 {
                     ...
                     var commitCount = 0;
                     var eventsCount = 0;
                     foreach (var commit in arStream.Commits)
                     {
                         commitCount++;
                         eventsCount += commit.Events.Count();
                     }
