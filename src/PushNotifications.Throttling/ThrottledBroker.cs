...
using PushNotification.Monitors;
using PushNotifications.Throttling.Logging;
...
namespace PushNotifications.Throttling
{
    public class ThrottledBroker : IDisposable
    {
        ...
        public ThrottledBroker(IContainer container, IPushBroker broker, ThrotlleSettings throtleSettings)
        {
        ...
        private IThrottleNotification ToThrottle(Notification notification)
        {
            ...
            if (notification is PushyNotification)
            {
                return new PushyNotificationMessage(notification as PushyNotification);
            }
        ...
        public class MessagePublishingWork : IWork
        {
            ...
            static ILog log = LogProvider.GetLogger(typeof(MessagePublishingWork));
            ProcessorMonitor monitor;
            ThrotlleSettings throtleSettings;

            public MessagePublishingWork(IEndpoint endpoint, Func<byte[], object> deserizalize, IPushBroker broker, ThrotlleSettings throtleSettings)
            {
                this.broker = broker;
            ...
            public void Start()
            {
                try
                {
                    ...
                    {
                        log.Warn("Usage above " + throtleSettings.PushNotificationsMaxCPUUtilization + "%, not sending notifications. Usage: " + usage + "%");
                    }
                }
                ...
                finally
                {
                    ...
                    {
                        log.ErrorException("Could not close the endpoint.", ex);
                    }
                    ScheduledStart = DateTime.UtcNow.AddSeconds(throtleSettings.PushNotificationsSendoutDelay);
                }
            }
