    ...
    public class NotificationSender : IDisposable
    {
        ...
        public void SendNotification(INotification notification)
        {
            ...
            if (!channels.ContainsKey(notificationType))
            {
                ...
                else if (notification is PushyNotification)
                {
                    broker.QueueNotification<PushyNotification>(notification as PushyNotification);
                }
                else
        ...
        private void SendNotificationCallbackDelegate(object sender, SendNotificationResult result)
        {
            if (result.IsSuccess)
            {
                log.Info("Success sending notification for token: " + GetNotificationToken(result) + "Notification " + result.Notification, result.Error);
            }
            else
            {
                log.Error("Fail to send notification  for token:" + GetNotificationToken(result) + "Error " + result.Error + "Notification " + result.Notification, result.Error);
            }
        }
        ...
        private string GetNotificationToken(SendNotificationResult result)
        {
            ...
            if (notification is AppleNotification)
                ...
                token = (notification as PushyNotification).Tokens.Single();
